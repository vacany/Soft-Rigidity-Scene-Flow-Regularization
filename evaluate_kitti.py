...
def generate_configs():
    import itertools
    permutations = {'dataset_type' : ['kitti_t', 'kitti_o'],
                    'lr': [0.001, 0.008],
                    'K': [32],
...
if __name__ == '__main__':
    ...
    exp_folder = EXP_PATH + f'/multi-rigid-flow/{cfg_int}'
    ...
    for run in range(cfg['runs']):
        ...
        # visualize_points3D(pc1[0], mask.argmax(dim=2)[0])
        # visualize_points3D(pc1[0], t[0,:].norm(dim=-1))
        #
        # visualize_points3D(pc1[0], yaw[0, :, 0])
        # breakpoint()
