...
from models.MBNSF.utils.general_utils import *
from models.MBNSF.utils.nsfp_utils import *
from models.MBNSF.utils.o3d_uitls import extract_clusters_dbscan
from models.MBNSF.utils.sc_utils import spatial_consistency_loss
...
def sceneflow_deep_prior(
    ...
    # visualize_points3D(pc1.detach().cpu(), labels_t.detach().cpu())
    ...
    DT_layer = DT(pc1.unsqueeze(0), pc2.unsqueeze(0))
    DT_layer_backward = DT(pc2.unsqueeze(0), pc1.unsqueeze(0))
    # import time
    # st = time.time()
    for epoch in range(options.iters):
        ...
        # breakpoint()
        # d_st = time.time()
        loss_chamfer_1, _ = DT_layer.torch_bilinear_distance(pc1_deformed.unsqueeze(0))
        # print("DT loss: ", time.time() - d_st)
        ...
        # print("SC loss: ", time.time() - st)

        if options.backward_flow:
            flow_pred_1_prime = net_inv(pc1_deformed)
            pc1_prime_deformed = pc1_deformed - flow_pred_1_prime
            loss_chamfer_1_prime, _ = DT_layer_backward.torch_bilinear_distance(pc1_prime_deformed.unsqueeze(0))
...
def fit_sequence_of_scene_flow_field(
    ...
    for i in range(n_lidar_sweeps-1):
        ...
        id_mask1 = inst_gt_list[i]
        # ANCHOR: sample points
        # num_pts = 8192
        # sample_idx = torch.randperm(min(pc1.shape[0],pc2.shape[0]))[:num_pts]
        # pc1 = pc1[sample_idx]
        # pc2 = pc2[sample_idx]
        # flow_gt_list[i] = flow_gt_list[i][sample_idx]
        # id_mask1 = inst_gt_list[i][sample_idx]

        # uncomment this to baseline
        id_mask1 = None
        ...
        flow_pred, info_dict_forward = sceneflow_deep_prior(pc1, pc2, info_dict, options, id_mask1)

        visualize_flow3d(pc1.detach().cpu(), pc2.detach().cpu(), flow_pred.detach().cpu())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_name', type=str, default='/mnt/personal/vacekpa2/experiments/fit_MBNSF_tl25_lr3_ep100', metavar='N', help='Name of the experiment.')
    parser.add_argument('--iters', type=int, default=2, metavar='N', help='Number of iterations to optimize the model.')#5000
    ...
    parser.add_argument('--dataset_path', type=str, default='/mnt/personal/vacekpa2/data/mbnsf-argoverse/av1_traj', metavar='N', help='Dataset path.')
    ...
    dataset = SFDataset4D(dataset_type='argoverse', only_first=True, n_frames=1)

    # for i, fi_id in enumerate(range(len(argoverse_tracking_val_log_ids))):
    ...
    for i in range(2,4):
        ...
        # todo bad annotations for argoverse flow? - pouzit Waymo

        # if not options.use_all_points:
        # for downsample of pts
        # options.num_points = 8192

        pc_list = [data['pc1'][0].numpy(), data['pc2'][0].numpy()]
        flow_gt_list = [data['gt_flow'][0].numpy()]
        inst_gt_list = [data['id_mask1'][0].numpy()]
        log_id = str(i)
        fi_id = i
        ...
        visualize_flow3d(pc_list[0], pc_list[1], flow_gt_list[0])

        cur_exp_dir = os.path.join(exp_dir_path, log_id)
        os.makedirs(cur_exp_dir, exist_ok=True)
        
        metrics = fit_sequence_of_scene_flow_field(cur_exp_dir, pc_list, options, flow_gt_list, inst_gt_list)
    ...
    # metric calculation divided by full dataset lenght, not actual
    # [seq_metrics[label].nonzero()] - indices to gather nonzero values - epe of 0 would not be counted
    seq_metrics_mean = {label:round(seq_metrics[label][seq_metrics[label].nonzero()].mean(), 4) for label in seq_metrics.keys()}
    logging.info('---------------------------------------')
